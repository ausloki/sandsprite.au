...
- `js/facilities.js` — fetches `data/facilities.csv`, groups photos by `Category`, and rebuilds the "Our facilities" grid on `services.html` in a fixed category order (Kitchenette, Boarding suites, Cattery, Daycare play yard, Outdoor runs, Large Day Pens, Small Day Pens). Each category shows as **one tile** (its first photo as the cover, with a photo count); clicking it opens an in-page lightbox with Prev/Next to browse every photo in that category (plus an "Open full size" link to the original file). A category with no rows yet shows the generic paw-icon placeholder tile and isn't clickable — it automatically becomes a clickable gallery as soon as you add its first CSV row. Falls back to the static tiles already in the HTML if the CSV can't be loaded or JS is off (the fallback links its cover photo straight to the full-size image instead, since the lightbox itself needs JS).
- `data/facilities.csv` — the facilities photo grid, one row per photo. Columns: `File,Category,Alt,Description`. `File` is the filename in `assets/facilities/`, `Category` must match one of the fixed category names above (add as many rows per category as you like — they all show up in that category's lightbox gallery), `Alt` is the accessibility text, `Description` is optional text shown under the photo in the lightbox.
- `assets/` — vector recreations of the kennel's real sign (walking figure + dogs), based on the owner's photo of the sign and confirmed against a second photo of it found on the kennel's Facebook page:
  ...
  - `paw.svg`, `hero-dog.svg` — original placeholders, no longer used for branding; `paw.svg` is still used as the generic photo-placeholder icon for the one remaining unfilled "Some of our dogs" card and any facilities category with no photos yet.
...
- `assets/facilities/` — real photos of the property (kitchenette, daycare play yard, outdoor runs, large/small day pens), shown via `data/facilities.csv` on the "Our facilities" section of `services.html`; click a category to browse all its photos in a lightbox.
